...
import com.demo.encapsulationokhttp.BuildConfig;
...
//        mRequestBuilder.addHeader("token","0F64A3776AC84482B285B02C6D0A0BA6");//可以把token添加到这儿
...
//        return formBody.build();
    ...
    private void setPostFile() {
        if(mFile != null && mFile.exists()) {
            ...
            Log.e("yml", "setPostParameAndFile: ......1 "+mfileKey+" "+mFile.getName()+" "+mFileType+" " );
        }else {
            Log.e("yml", "setPostFile: "+"文件不存在" );
        }
    }
    ...
    private void setPostParameAndFile() {
        if(mParamsMap != null && mFile != null){
            ...
            Log.e("yml", "setPostParameAndFile: .....2"+mfileKey+" "+mFile.getName()+" "+mFileType+" " );
        }
    }
    ...
    void execute(){
        build.newCall(mOkHttpRequest).enqueue(new Callback() {
            @Override
    ...
    private static class ProgressRequestBody extends RequestBody {
        ...
        public void writeTo(BufferedSink sink) throws IOException {
            //使用logging这里会造成连续读写两次造成字节过大上传失败
